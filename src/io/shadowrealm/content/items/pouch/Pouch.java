package io.shadowrealm.content.items.pouch;
...
public abstract class Pouch {
    ...
    public boolean configurationPermitted() {
        if (player.getPosition().inDuelArena() || player.getPosition().inPcGame() || player.getPosition().inPcBoat() || player.getPosition().isInJail() || player.getPA().viewingOtherBank || player.isDead || player.getLootingBag().isWithdrawInterfaceOpen() || player.getLootingBag().isDepositInterfaceOpen()) {
            return false;
        }
