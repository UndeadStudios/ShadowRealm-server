...
import io.shadowrealm.content.PetCollector;
import io.shadowrealm.content.PlatinumTokens;
...
import io.shadowrealm.content.bosses.bryophyta.BryophytaNPC;
...
import io.shadowrealm.content.objects.BondFIres;
import io.shadowrealm.content.objects.Fillables;
...
import io.shadowrealm.content.skills.herblore.Crushable;
import io.shadowrealm.content.skills.herblore.HerbloreDecantCleanUnfNpc;
import io.shadowrealm.content.skills.herblore.PoisonedWeapon;
import io.shadowrealm.content.skills.herblore.PotionDecanting;
import io.shadowrealm.content.skills.prayer.Bone;
import io.shadowrealm.content.skills.prayer.Prayer;
import io.shadowrealm.content.skills.runecrafting.Tiaras;
import io.shadowrealm.content.skills.slayer.SlayerUnlock;
import io.shadowrealm.content.trails.MasterClue;
...
import io.shadowrealm.util.logging.player.ItemOnItemLog;
...
//@sniper or @epic DO NOT TORCH THIS.. SGS.. THE OLD CODE BUGS OUT ITEM ON OBJECT AKA FARMING ALSO
		...
		//this is broken. if u use a item on a diffent object then it crashs the client
/*		if (c.getItems().isNoted(itemId) || (def!=null ? def.name : null) != null && def.name.toLowerCase().contains("bank") || def.name.toLowerCase().contains("trading") || def.name.toLowerCase().contains("bank booth")) {
			c.getPA().sendEnterAmount(0);
			c.unNoteItemId = itemId;
			c.settingUnnoteAmount = true;
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (itemUsed == 233 && useWith == 5075 || itemUsed == 5075 && useWith == 233) {
			if (c.getItems().playerHasItem(233) && c.getItems().playerHasItem(5075)) {
				int count = c.getItems().getInventoryCount(233);
				int count2 = c.getItems().getInventoryCount(5075);
				int lowestAmount = count;
				if (count > count2)
					lowestAmount = count2;
				//c.getItems().deleteItem(233, lowestAmount);
				c.getItems().deleteItem(5075, lowestAmount);
				c.getItems().addItem(6693, lowestAmount);
			}
		}
