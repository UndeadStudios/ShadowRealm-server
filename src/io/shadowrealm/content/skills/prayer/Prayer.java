package io.shadowrealm.content.skills.prayer;
        ...
        Server.getEventHandler().submit(new Event<Player>("skilling", player, 3) {
            ...
            public void execute() {
                ...
                if (player.getPosition().inWild() && chance == 1) {
                    ...
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.WILDYALTAR1.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.WILDYALTAR2.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.WILDYALTAR3.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                } else {
                    // IF NOT IN THE WILDERNESS.....
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.ALTAR1.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.ALTAR2.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                    if (player.currentDailyTask.getTaskName().equals(DailyTaskData.ALTAR3.getDailyTask().getTaskName())) {
                        DailyTaskHandler.Companion.handleProgress(player, 1);
                    }
                }
                player.getItems().deleteItem2(bone.getItemId(), 1);
