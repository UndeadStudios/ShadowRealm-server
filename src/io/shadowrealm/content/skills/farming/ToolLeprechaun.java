package io.shadowrealm.content.skills.farming;
...
public class ToolLeprechaun {
    ...
    public List<GameItem> storeItems2Client = Arrays.asList(new GameItem[]{new GameItem(1925), new GameItem(6032),
            new GameItem(6034)});
    ...
    public void loadInterfaces() {
        try {
            ...
            player.getPA().sendItems(player, LEPRECHAUN_INTERFACE_CONTAINER, storeItems, 25);
            player.getPA().sendItems(player, LEPRECHAUN_INTERFACE_CONTAINER2,
                    storeItems2,25 );
            player.setSidebarInterface(3, PLAYER_INTERFACE);
            player.getPA().sendItems(player, PLAYER_INTERFACE_CONTAINER,
                    storeItemsClient,25);
            player.getPA().sendItems(player, PLAYER_INTERFACE_CONTAINER2,
                   storeItems2Client, 25);
            updateStore();
        } catch(Exception e){
            e.printStackTrace();
        }
    ...
    public void handleAdditionalTools() {
        ...
        storeItemsClient.set(4, new GameItem(item));

        if (player.getItems().playerHasItem(7409)) {
            storeItemsClient.set(3, new GameItem(7409));
        } else {
            storeItemsClient.set(3, new GameItem(5329));
        }
    ...
    public void checkWateringCanQuantity() {
        ...
        if (counter == 0) {
            storeItems.set(4, new GameItem(5331));
        }
        if (counter2 == 0) {
            storeItemsClient.set(4, new GameItem(5331));
        }
    ...
    public void updateStore() {
        ...
        for (int i = 0; i < tools.length; i++) {
            ...
            if (toolStoreData.getToolId() != 5332
                ...
                storeItems.set(4, new GameItem(toolStoreData.getToolId()));
            }
            if (toolStoreData.getToolId() == 7409 && tools[i] == 1) {
                storeItems.set(3, new GameItem(7409));
            }
        ...
        player.getPA().sendItems(player, LEPRECHAUN_INTERFACE_CONTAINER,
                storeItems, 25);
        player.getPA().sendItems(player, PLAYER_INTERFACE_CONTAINER,
                storeItemsClient, 25);
    ...
    public boolean noteItem(int itemId) {
        ...
        for (int item : NOTABLE_ITEMS) {
            if (itemId == item) {
                ...
                player.getItems().deleteItem2(itemId, count);
