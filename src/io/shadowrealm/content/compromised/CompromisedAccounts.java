package io.shadowrealm.content.compromised;
...
public class CompromisedAccounts {
    ...
    public static void init() {
        try {
            if (!Server.isPublic())
                return;
            File file = new File(Server.getSaveDirectory() + "exilius_Compromised_Accounts.json");
            if (!file.exists()) {
    ...
    private static boolean message(Player player) {
        ...
        if (type != null) {
            ...
            String[] message = {
                    "Although shadow realm has not been compromised, it has been determined",
