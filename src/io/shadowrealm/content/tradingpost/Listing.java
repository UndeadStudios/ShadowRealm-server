package io.shadowrealm.content.tradingpost;
...
import io.shadowrealm.util.logging.player.TradingPostBuyLog;
...
public class Listing {
	...
	public static void displayResults(List<Sale> sales, Player c, Boolean loadRecent) {
		...
		int total = 0, skipped = 0, start = 44023;
		/** Sort highest price **/
		if (!loadRecent) {
		...
		PlayerAssistant.sendItems(c, 44022, result, 250);
		for(int i = start; i < 44023; i++) {
			c.getPA().sendFrame126("", i);
		}
		c.saleResults = idk;
	}
