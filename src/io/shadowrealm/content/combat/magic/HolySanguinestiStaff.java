package io.shadowrealm.content.combat.magic;
...
public class HolySanguinestiStaff {
    ...
    public static boolean useItem(Player player, int item1, int item2) {
        if (item1 == Items.BLOOD_RUNE && item2 == Items.HOLY_SANGUINESTI_STAFF || item2 == Items.BLOOD_RUNE && item1 == Items.HOLY_SANGUINESTI_STAFF) {
            charge(player);
            return true;
        }
    ...
    private static void charge(Player player) {
        if (player.getItems().playerHasItem(Items.HOLY_SANGUINESTI_STAFF)) {
