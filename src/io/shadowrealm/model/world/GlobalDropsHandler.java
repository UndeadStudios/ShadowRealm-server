...
import io.shadowrealm.util.GlobalDropData;
...
public class GlobalDropsHandler {
    ...
    public static void initialize() {
        ...
        try {
            ...
            GlobalDropData[] globalDropData = gson.fromJson(new FileReader(Server.getDataDirectory()+"/cfg/globaldrops.json"), collectionType);
        ...
        for (Player player : PlayerHandler.players) {
            ...
            if (player2 != null) {
                CycleEventHandler.getSingleton().addEvent(player2, new CycleEvent() {
                    @Override
                            ...
                            if (drop.isTaken() && drop.isSpawned()) {
                                if (System.currentTimeMillis() - drop.getTakenAt() >= TIME_TO_RESPAWN * 1000) {
                                    ...
                                    if (player2.distanceToPoint(drop.getX(), drop.getY()) <= 60) {
                                        player2.sendMessage("gd"+drop.getId());
                                        Server.itemHandler.createGroundItem(player2, drop.getId(), drop.getX(), drop.getY(), drop.getHeight(), drop.getAmount());
    ...
    public static void pickup(Player player, int a, int b, int c) {
        ...
        for (Player playerLoop : PlayerHandler.players) {
            ...
            if (cl != null) {
                Server.itemHandler.removeGroundItem(cl, drop.getId(), drop.getX(), drop.getY(), drop.getHeight(), true);
    ...
    public static void load(Player player) {
        for (GlobalDrop drop : globalDrops) {
            if (!drop.isTaken() && !drop.isSpawned() && !itemExists(drop.getId(), drop.getX(), drop.getY(), true) && player.distanceToPoint(drop.getX(), drop.getY()) <= 60) {
                Server.itemHandler.createGroundItem(player, drop.getId(), drop.getX(), drop.getY(), drop.getHeight(), drop.getAmount());
                spawnedDrops.add(drop);
    ...
    public static void reset(Player c) {
        for (GlobalDrop drop : globalDrops) {
            if (c.distanceToPoint(drop.getX(), drop.getY()) <= 60) {
                Server.itemHandler.removeGroundItem(c, drop.getId(), drop.getX(), drop.getY(), drop.getHeight(), true);
            }
        }
        spawnedDrops.clear();
        globalDrops.clear();
        ...
        for (GlobalDrop drop : globalDrops) {
            if (!drop.isTaken() && !drop.isSpawned() && !itemExists(drop.getId(), drop.getX(), drop.getY(), true) && c.distanceToPoint(drop.getX(), drop.getY()) <= 60) {
                Server.itemHandler.createGroundItem(c, drop.getId(), drop.getX(), drop.getY(), drop.getHeight(), drop.getAmount());
                spawnedDrops.add(drop);
                drop.setSpawned(true);
            }
        }
