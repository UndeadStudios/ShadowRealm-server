...
import io.shadowrealm.content.achievement_diary.AchievementDiaryKills;
import io.shadowrealm.content.bosses.Cerberus;
import io.shadowrealm.content.bosses.RevenantMaledictus;
...
import io.shadowrealm.content.minigames.xeric.XericWave;
import io.shadowrealm.content.questing.hftd.DagannothMother;
...
import io.shadowrealm.model.entity.npc.actions.LoadSpell;
import io.shadowrealm.model.entity.npc.data.AttackAnimation;
...
public static void addNPC(int npcType, int x, int y, int h) {
    ...
    public static boolean isFightCaveNpc(NPC npc) {
        ...
        switch (npc.getNpcId()) {
        case io.shadowrealm.content.minigames.fight_cave.Wave.TZ_KEK_SPAWN:
        case io.shadowrealm.content.minigames.fight_cave.Wave.TZ_KIH:
        case io.shadowrealm.content.minigames.fight_cave.Wave.TZ_KEK:
        case io.shadowrealm.content.minigames.fight_cave.Wave.TOK_XIL:
        case io.shadowrealm.content.minigames.fight_cave.Wave.YT_MEJKOT:
        case io.shadowrealm.content.minigames.fight_cave.Wave.KET_ZEK:
        case Wave.TZTOK_JAD: 
            return true;
        }
        return false;
    ...
    public void stepAway2(Player player, NPC npc) {
        int[][] points = { { -1, 0 }, { 1, 0 }, { 0, -1 }, { 0, 1 } };

        for (int[] k : points) {
            int dir = NPCClipping.getDirection(k[0], k[1]);
            if (NPCDumbPathFinder.canMoveTo(npc, dir)) {
                NPCDumbPathFinder.walkTowards(npc, npc.absX > player.absX ? player.absX-1 : player.absX+1 ,npc.absY > player.absY ? player.absY+1 : player.absY-1);
                break;
            }
        }
    }
