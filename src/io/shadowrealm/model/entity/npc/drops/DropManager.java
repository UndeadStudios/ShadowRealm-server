package io.shadowrealm.model.entity.npc.drops;
                ...
                myMaster.ifPresent(m -> {
                    ...
                    if (npc.getPosition().inWild() && m.getId() == 7663) {
                        ...
                        if (Misc.random(emblemChance) == 1) {
                            ...
                            Discord.raredropMessage(player.getDisplayName() + ": has received " + ItemDef.forId(1276).getName());
                        }
                        if (Misc.random(slayerChance) == 1) {
                            ...
                            Discord.raredropMessage(player.getDisplayName() + ": has received " + ItemDef.forId(21297).getName());
...
//        }
    ...
    public void create(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        group.ifPresent(g -> {
            ...
            for (GameItem item : drops) {
                ...
                ItemDef Def = itemDef;
                if (Def.getShopValue() > 25_000_000) {
                    Discord.raredropMessage(player.getDisplayName() + ": has received " + ItemDef.forId(item.getId()).getName());
                }
            }
            handle(player, npc, location, repeats, npcId);
        });
    ...
    public static double getModifier(Player player) {
       ...
       // modifier += player.getMode().getDropModifier();
        ...
        if (player.getItems().isWearingItem(29235)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29236)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29237)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29239)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29240)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29241)) {
            modifier -= .05;
        }
        if (player.getItems().isWearingItem(29242)) {
            modifier -= .05;
        }
            ...
            for (TablePolicy policy : TablePolicy.POLICIES) {
                ...
                if (table.isPresent()) {
                    ...
                    if (player.getRights().isOrInherits(Right.OWNER)) {
                        rate = (int) Math.ceil((table.get().getAccessibility() * modifier)) * table.get().size(); // Effective rate
                    }
