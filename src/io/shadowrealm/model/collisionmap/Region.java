package io.shadowrealm.model.collisionmap;
...
public static void dumpDoorobject(int objectId, int x, int y, int face, int h, int type) {
   ...
   //Server.npcHandler.spawnNpc(c, npcType, absX, absY, heightLevel, 1, 120, 7, 70, 70, false, false);
    ...
    public void addObject(int objectId, int x, int y, int height, int type, int direction) {
        ...
        if ((def != null ? def.name : null) != null && def.name.equalsIgnoreCase("trapDoor")  && def.actions[0].equalsIgnoreCase("open")) {
...
//	}
    ...
    private static void loadMaps(int regionId, ByteStream str1, ByteStream str2) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int i2 = 0; i2 < 64; i2++) {
                for (int i3 = 0; i3 < 64; i3++) {
                    if ((tileFlags[i][i2][i3] & 1) == 1) {
                        int height = i;
                        if ((tileFlags[1][i2][i3] & 2) == 2) {
                            height--;
                        }
        ...
        int objectId = -1;
        do {
            int i1 = str1.method1606();
            if (i1 == 0)
                break;
            objectId += i1;
            int location = 0;
            do {
                int k1 = str1.readUShortSmart();
                if (k1 == 0)
                    break;
                location += k1 - 1;
